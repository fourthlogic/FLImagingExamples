...
int main()
{
	...
	do
	{
		...
		if((eResult =rollingLeft.Execute()))
		{
		...
		if((eResult =arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(BitRollingLeft 1)", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to draw text\n");
			break;
		}

		if((eResult =arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(BitRollingLeft 7)", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to draw text\n");
			break;
		}
