...
int main()
{
	...
	do
	{
		...
		if((result = arrFliImage[EType_Source].Load(L"../../ExampleImages/NoiseImage/NoiseImage1.flif")).IsFail())
		{
		...
		if((result = arrViewImage[EType_Source].Create(100, 0, 670, 427)).IsFail())
		{
		...
		if((result = arrViewImage[EType_Destination1].Create(670, 0,1240, 427)).IsFail())
		{
			ErrorPrint(result, L"Failed to create the image view.\n");
			break;
		}
		...
		if((result = arrViewImage[EType_Destination2].Create(1240, 0,1810, 427)).IsFail())
		{
			ErrorPrint(result, L"Failed to create the image view.\n");
			break;
		}
		...
		// CSigmaFilter 객체 생성 // Create CSigmaFilter object
		CSigmaFilter sigmaFilter;
		// Source 이미지 설정 // Set source image
		sigmaFilter.SetSourceImage(arrFliImage[EType_Source]);
		...
		if((result = sigmaFilter.Execute()).IsFail())
		{
		...
		if((result = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPoint<double>(0, 0), L"SigmaFilter Size : 31x31 Sigma : 3", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPoint<double>(0, 0), L"SigmaFilter Size : 101x101 Sigma: 3", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}
