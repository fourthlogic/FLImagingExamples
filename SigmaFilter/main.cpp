...
int main()
{
	...
	do
	{
		...
		if((result = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"SigmaFilter Size : 31x31 Sigma : 3", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"SigmaFilter Size : 101x101 Sigma: 3", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}
