...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = sBRI.Execute()))
		{
			ErrorPrint(eResult, "Failed to execute Bilinear Region Interpolation.");
			break;
		}
		...
		if(IsFail(eResult = arrLayer[EType_Destination].DrawTextCanvas(&CFLPointD(0, 0), L"Destination Image (Bilinear)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
