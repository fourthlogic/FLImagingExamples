...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationDivide/Moon.flif")))
		{
		...
		if(IsFail(res = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationDivide/FlowerWall.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
