...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionMode2Dto1D/Source.flif")))
		{
		...
		// Reduce Dimension Mode 2D to 1D 객체 생성 // Create Reduce Dimension Mode 2D to 1D object
		CReduceDimensionMode2Dto1D reduceDimensionMode2Dto1D;

		// Source 이미지 설정 // Set source image 
		reduceDimensionMode2Dto1D.SetSourceImage(fliSourceImage);

		// Destination 이미지 설정 // Set destination image
		reduceDimensionMode2Dto1D.SetDestinationImage(fliDestinationImageX);

		// 축소 차원 설정 // Set reduction dimension
		reduceDimensionMode2Dto1D.SetReductionDimension(CReduceDimensionMode2Dto1D::EReductionDimension_X);

		// 알고리즘 수행 // Execute the algorithm
		if((res = reduceDimensionMode2Dto1D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension Mode 2D to 1D.\n");

			break;
		}

		// Destination 이미지 설정 // Set destination image
		reduceDimensionMode2Dto1D.SetDestinationImage(fliDestinationImageY);

		// 축소 차원 설정 // Set reduction dimension
		reduceDimensionMode2Dto1D.SetReductionDimension(CReduceDimensionMode2Dto1D::EReductionDimension_Y);
