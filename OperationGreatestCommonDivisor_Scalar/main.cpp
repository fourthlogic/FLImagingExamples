...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationGreatestCommonDivisor/Sunset.flif")))
		{
		...
		if(IsFail(eResult = gcd.Execute()))
		{
			ErrorPrint(eResult, "Failed to execute operation GCD.");
			break;
		}
		...
		if(IsFail(eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(gcd 50)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if(IsFail(eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image(gcd 200)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
