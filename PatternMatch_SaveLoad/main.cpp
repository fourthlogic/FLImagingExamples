...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(eResult = fliLearnImage.Load(L"../../ExampleImages/Matching/Pattern Single Learn.flif")))
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(eResult = fliFindImage.Load(L"../../ExampleImages/Matching/Pattern Single Find.flif")))
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}
		...
		if(IsFail(eResult = FLPatternMatchSave.Learn()))
		{
		...
		if(IsFail(eResult = FLPatternMatchSave.Save(L"../../ExampleImages/Matching/Pattern Single Learn File")))
		{
		...
		if(IsFail(eResult = FLPatternMatchLoad.Load(L"../../ExampleImages/Matching/Pattern Single Learn File")))
		{
			ErrorPrint(eResult, "Failed to save\n");
			break;
		}
		...
		// 알고리즘 수행 // Execute the algorithm
		eResult = FLPatternMatchLoad.Execute();
