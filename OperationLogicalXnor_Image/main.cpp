...
int main()
{
	...
	do
	{
		...
		if((eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationLogicalXnor/Cat.flif")).IsFail())
		{
		...
		if((eResult = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationLogicalXnor/Space.flif")).IsFail())
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}
		...
		{
			ErrorPrint(eResult, "Failed to execute operation logicalXnor.");
			break;
		}
