...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationAbsolute/Flower.flif")))
		{
		...
		if((eResult = absolute.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation absolute.");
			break;
		}
