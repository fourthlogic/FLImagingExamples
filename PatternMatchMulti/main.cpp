...
int main()
{
	...
	CPatternMatchMulti FLPatternMatchMulti;
	...
	do
	{
		...
		for(int64_t i64DataIdx = 0; i64DataIdx < 2; ++i64DataIdx)
		{
			...
			if(IsFail(eResult = FLPatternMatchMulti.Learn(arrClassName[i64DataIdx])))
			{
		...
		FLPatternMatchMulti.SetAngleTolerance(10.);
		...
		FLPatternMatchMulti.SetMaxObjectMode(CPatternMatchMulti::EMaxObjectMode_Total);
		...
		FLPatternMatchMulti.EnableInterpolation();
		...
		// 알고리즘 수행 // Execute the algorithm
		eResult = FLPatternMatchMulti.Execute();
