...
int main()
{
	...
	do
	{
		...
		int32_t i32ResultTypeMask = (int32_t)CBlob::EBlobResultType_Contour | (int32_t)CBlob::EBlobResultType_Area;
		...
		// 면적이 100보다 작은 객체들을 제거(작은 노이즈 제거)
		if(IsFail(eResult = blob.Filter(CBlob::EFilterItem_Area, 100, ELogicalCondition_Less)))
		{
			ErrorPrint(eResult, "Blob filtering algorithm error occurred.");
			break;
		}

		// Blob 결과를 얻어오기 위해 CFLFigureArray / CFLArray<uint64_t> 선언
		CFLFigureArray flfaContours;
		CFLArray<uint64_t> flaArea;
		...
		if(IsFail(eResult = blob.GetResultAreas(flaArea)))
		{
			ErrorPrint(eResult, "Failed to get area from the Blob object.");
			break;
		}
		...
		// Image View 객체에 Index, Area 순서대로 출력
