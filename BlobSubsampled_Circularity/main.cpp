...
int main()
{
	...
	do
	{
		...
		// BlobSubsampled Result Type mask 생성 (Contour, Circularity)
		int32_t i32ResultTypeMask = (int32_t)CBlobSubsampled::EBlobResultType_Contour | (int32_t)CBlobSubsampled::EBlobResultType_Circularity;
		...
		if(IsFail(res = blob.Filter(CBlobSubsampled::EFilterItem_Circularity, 0.85, ELogicalCondition_Less)))
		{
			ErrorPrint(res, "BlobSubsampled filtering algorithm error occurred.");
			break;
		}

		// BlobSubsampled 결과를 얻어오기 위해 CFLFigureArray / CFLArray<double> 선언
		...
		// BlobSubsampled 결과들 중 Circularity 을 얻어옴
		if(IsFail(res = blob.GetResultCircularities(flaCircularity)))
		{
			ErrorPrint(res, "Failed to get circularities from the BlobSubsampled object.");
			break;
		}
		...
		// Image View 객체에 Index,  Circularity 순서대로 출력
