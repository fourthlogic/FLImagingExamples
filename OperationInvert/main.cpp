...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationInvert/board.flif")))
		{
		...
		if((res = Invert.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute operation Invert.");
			break;
		}
