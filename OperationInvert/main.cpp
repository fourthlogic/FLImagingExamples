...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationInvert/board.flif")))
		{
		...
		// Operation Invert 객체 생성 // Create Invert object
		COperationInvert Invert;
		// Source 이미지 설정 // Set source image
		Invert.SetSourceImage(arrFliImage[EType_Source]);
		// Destination 이미지 설정 // Set destination image 
		Invert.SetDestinationImage(arrFliImage[EType_Destination]);

		// 알고리즘 수행 // Execute the algorithm
		if((eResult = Invert.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation Invert.");
			break;
		}
