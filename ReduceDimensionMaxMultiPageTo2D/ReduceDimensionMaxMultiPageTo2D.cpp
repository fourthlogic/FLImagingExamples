...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionMaxMultiPageTo2D/Source.flif")))
		{
		...
		// Reduce Dimension Max Multi Page To 2D 객체 생성 // Create Reduce Dimension Max Multi Page To 2D object
		CReduceDimensionMaxMultiPageTo2D reduceDimensionMaxMultiPageTo2D;

		// Source 이미지 설정 // Set source image 
		reduceDimensionMaxMultiPageTo2D.SetSourceImage(fliSourceImage);

		// Destination 이미지 설정 // Set destination image
		reduceDimensionMaxMultiPageTo2D.SetDestinationImage(fliDestinationImage);

		// 알고리즘 수행 // Execute the algorithm
		if((res = reduceDimensionMaxMultiPageTo2D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension Max Multi Page To 2D.\n");
