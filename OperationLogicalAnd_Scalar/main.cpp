...
int main()
{
	...
	do
	{
		...
		if((eResult = fliSourceImage.Load(L"../../ExampleImages/OperationLogicalAnd/Cat.flif")).IsFail())
		{
		...
		if((eResult = logical.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation logical and.");
			break;
		}
