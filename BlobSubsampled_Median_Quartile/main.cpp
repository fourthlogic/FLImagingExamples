...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = blob.Filter(CBlob::EFilterItem_BoundaryRectWidth, 20, ELogicalCondition_LessEqual)))
		{
		...
		for(int64_t i = 0; i < i64ResultCount; ++i)
		{
			...
			if(IsFail(eResult = imgStatistics.GetMedian(mvMedian)))
			{
				ErrorPrint(eResult, "Failed to get Median Value from the Blob object.");
				break;
			}
			...
			if(IsFail(eResult = imgStatistics.GetLowerQuartile(mvLowerQuartile)))
			{
				ErrorPrint(eResult, "Failed to get LowerQuartile Value from the Blob object.");
				break;
			}
			...
			if(IsFail(eResult = imgStatistics.GetUpperQuartile(mvUpperQuartile)))
			{
				ErrorPrint(eResult, "Failed to get UpperQuartile Value from the Blob object.");
				break;
			}
