...
int main()
{
	...
	do
	{
		...
		if((eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/LaplacianOfGaussianFilter/Gear.flif")).IsFail())
		{
		...
		LaplacianOfGaussianFilter.SetSigma(0.5);
 		...
 		if((eResult = LaplacianOfGaussianFilter.Execute()).IsFail())
 		{
		...
		if((eResult = LaplacianOfGaussianFilter.Execute()).IsFail())
		{
			ErrorPrint(eResult, L"Failed to execute gauss filter.");
			break;
		}
