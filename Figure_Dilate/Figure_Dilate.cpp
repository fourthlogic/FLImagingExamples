...
int main()
{
	...
	do
	{
		...
		layer[1].DrawTextCanvas(&CFLPoint<int32_t>(0, 0), L"Dilate Result1", YELLOW, BLACK, 30);
		layer[2].DrawTextCanvas(&CFLPoint<int32_t>(0, 0), L"Dilate Result2", YELLOW, BLACK, 30);
		layer[3].DrawTextCanvas(&CFLPoint<int32_t>(0, 0), L"Dilate Result3", YELLOW, BLACK, 30);
		...
		if((res = flrgSourceFig.Dilate(&fleKernel, &flfaResult3)).IsFail())
		{
			ErrorPrint(res, "Failed to calculate.\n");
			break;
		}
