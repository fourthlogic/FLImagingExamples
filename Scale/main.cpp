...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = arrLayer[EType_Dst1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination Image1 (x1.5)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = arrLayer[EType_Dst2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination Image2 (x1.5 Resize)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = arrLayer[EType_Dst3].DrawTextCanvas(&CFLPointD(0, 0), L"Destination Image3 (1024x1024 Resize)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
