...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/OperationSoftShrinkage/Coord1D.flif")))
		{
		...
		// Operation SoftShrinkage 객체 생성 // Create Operation SoftShrinkage object
		COperationSoftShrinkage algObject;

		// 이미지 설정 // Set the images
		algObject.SetSourceImage(fliSourceImage);
		algObject.SetDestinationImage(fliDestinationImage0);
		algObject.SetOperationMode(COperationSoftShrinkage::EOperationMode_Forward);
		...
		algObject.SetOperationMode(COperationSoftShrinkage::EOperationMode_Backward);
