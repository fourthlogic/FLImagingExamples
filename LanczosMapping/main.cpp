...
int main()
{
	...
	do
	{
		...
		viewImage[0].SetOffset(&CFLPoint<double>(f64Offset, f64Offset));
		...
		// 계속 새로 데이터를 생성하는것을 테스트 하려 한다면 아래 Load함수와 관련된 if문 1줄을 삭제하면 됩니다.
		...
		if(IsFail(res = lczm.Load(L"MappingData.fllcz")))
		{
			...
			lczm.Initialize(flpGridSize, i32Extension);
		...
		for(int32_t y = 0; y < lczm.GetRow(); ++y)
		{
			...
			{
				const CLanczosMapping::CLanczosMappingVertexInfo* pVertex = lczm.GetControlPoint(CFLPointL(x, y));
		...
		for(int y = 0; y <= (lczm.GetRow() - 1) * f64Slice; ++y)
		{
			flpSource.y = (y / f64Slice);

			for(int x = 0; x <= (lczm.GetColumn() - 1) * f64Slice; ++x)
			{
				...
				if(IsOK(lczm.ConvertSourceToTarget(flpSource, flpTarget)))
				{
					...
					if(IsOK(lczm.ConvertTargetToSource(flpTarget, flpConvertedSource)))
					{
