	...
	do
	{
		...
		if(bAutoDetect)
		{
			...
			case CDeviceGenICamBase::EDeviceType_GigE:
				drReturn = camPylon.GetAutoDetectGigECameraSerialNumbers(flarrSerialNumbers);
				break;

			case CDeviceGenICamBase::EDeviceType_IEEE1394:
				drReturn = camPylon.GetAutoDetectIEEE1394CameraSerialNumbers(flarrSerialNumbers);
				break;

			case CDeviceGenICamBase::EDeviceType_USB:
				drReturn = camPylon.GetAutoDetectUSBCameraSerialNumbers(flarrSerialNumbers);
				break;

			case CDeviceGenICamBase::EDeviceType_CameraLink:
				drReturn = camPylon.GetAutoDetectCameraLinkCameraSerialNumbers(flarrSerialNumbers);
				break;
		...
		// 카메라에 이벤트 객체 설정
		camPylon.RegisterDeviceEvent(&eventImage);

		// 카메라에 장치 타입 설정
		camPylon.SetDeviceType(eDeviceType);
		...
		if(bAutoDetect)
		{
			...
			{
			case CDeviceGenICamBase::EDeviceType_GigE:
				drReturn = camPylon.AutoDetectGigECamera(i32SelectDevice);
				break;

			case CDeviceGenICamBase::EDeviceType_IEEE1394:
				drReturn = camPylon.AutoDetectIEEE1394Camera(i32SelectDevice);
				break;

			case CDeviceGenICamBase::EDeviceType_USB:
				drReturn = camPylon.AutoDetectUSBCamera(i32SelectDevice);
				break;

			case CDeviceGenICamBase::EDeviceType_CameraLink:
				drReturn = camPylon.AutoDetectCameraLinkCamera(i32SelectDevice);
				break;
		...
		{
			// 장치 연결 방법을 설정합니다.
			camPylon.SetConnectionMethod(eConnectionMethod);

			if(eConnectionMethod == CDeviceGenICamBase::EConnectionMethod_SerialNumber)
				// 시리얼 번호를 설정합니다.
				camPylon.SetSerialNumber(flsConnection);
			else
				// IP 주소를 설정합니다.
				camPylon.SetIPAddress(flsConnection);
		}

		// 카메라를 초기화 합니다.
		drReturn = camPylon.Initialize();
		if(drReturn.IsFail())
		{
