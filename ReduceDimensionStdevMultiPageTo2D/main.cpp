...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionStdevMultiPageTo2D/Source.flif")))
		{
		...
		// Reduce Dimension Stdev Multi Page To 2D 객체 생성 // Create Reduce Dimension Stdev Multi Page To 2D object
		CReduceDimensionStdevMultiPageTo2D reduceDimensionStdevMultiPageTo2D;

		// Source 이미지 설정 // Set source image 
		reduceDimensionStdevMultiPageTo2D.SetSourceImage(fliSourceImage);

		// Destination 이미지 설정 // Set destination image
		reduceDimensionStdevMultiPageTo2D.SetDestinationImage(fliDestinationImage);

		// 알고리즘 수행 // Execute the algorithm
		if((res = reduceDimensionStdevMultiPageTo2D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension Stdev Multi Page To 2D.\n");
