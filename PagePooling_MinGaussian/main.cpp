...
int main()
{
	...
	{
		// 이미지 로드 // Load image
		if((res = fliSourceImage.Load(L"../../ExampleImages/PagePooling/Multiple File_Min.flif")).IsFail())
		{
		...
		//	- Mean : 입력된 이미지들의 평균 값을 출력합니다. (최대 16843009 장 까지 지원됩니다.) // Mean: Outputs the average value of the entered images. (Up to 16843009 pages are supported.)
  		pagePooling.SetSamplingMethod(CPagePooling::ESamplingMethod_MinGaussian);
