...
int main()
{
	...
	do
	{
		...
		if((res =arrLayer[EType_Operand].DrawTextCanvas(&flpZero, L"Operand Image", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(res, L"Failed to draw text\n");
			break;
		}

		if((res =arrLayer[EType_Destination].DrawTextCanvas(&flpZero, L"Destination Image", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(res, L"Failed to draw text\n");
			break;
		}
