...
int main()
{
	...
	do
	{
		...
		if((eResult =arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationBitRolling/wave.flif")).IsFail())
		{
		...
		if((eResult =arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationBitRolling/smoke.flif")).IsFail())
		{
			ErrorPrint(eResult, L"Failed to load the image file.\n");
			break;
		}
		...
		if((eResult =arrFliImage[EType_Destination].Assign(arrFliImage[EType_Source])).IsFail())
		{
		...
		rollingLeft.SetOperandImage(arrFliImage[EType_Operand]);
		// Destination 이미지 설정 // Set the destination image
		rollingLeft.SetDestinationImage(arrFliImage[EType_Destination]);
		// Operation source를 이미지로 설정 // Set operation source to image
		rollingLeft.SetOperationSource(EOperationSource_Image);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if((eResult =rollingLeft.Execute()).IsFail())
		{
			ErrorPrint(eResult, L"Failed to execute operation rolling.");
			break;
		}
