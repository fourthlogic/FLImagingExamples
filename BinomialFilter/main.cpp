...
int main()
{
	...
	do
	{
		...
		if((eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/BinomialFilter/notredame.flif")).IsFail())
		{
		...
		CFLRectL flrROI(200, 200, 500, 500);
 		...
 		CBinomialFilter BinomialFilter;
 		// Source 이미지 설정 // Set the source image
 		BinomialFilter.SetSourceImage(arrFliImage[EType_Source]); 
 		// Source ROI 설정 // Set the source ROI
 		BinomialFilter.SetSourceROI(flrROI);

 		// Destination1 이미지 설정 // Set the destination1 image
 		BinomialFilter.SetDestinationImage(arrFliImage[EType_Destination1]);
		...
		BinomialFilter.SetKernel(3);
 		...
 		if((eResult = BinomialFilter.Execute()).IsFail())
 		{
		...
		BinomialFilter.SetDestinationImage(arrFliImage[EType_Destination2]);
		...
		BinomialFilter.SetDestinationImage(arrFliImage[EType_Destination3]);
		// Destination3 ROI 설정
		BinomialFilter.SetDestinationROI(flrROI);
		...
		BinomialFilter.SetDestinationImage(arrFliImage[EType_Destination4]);
		// Destination4 ROI 설정
		BinomialFilter.SetDestinationROI(flrROI);
		...
		if((eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image (Kernel Size 3x3)", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to draw text\n");
			break;
		}

		if((eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image (Kernel Size 5x5)", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to draw text\n");
			break;
		}

		if((eResult = arrLayer[EType_Destination3].DrawTextCanvas(&CFLPointD(0, 0), L"Destination3 Image (Kernel Size 7x7)", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to draw text\n");
			break;
		}

		if((eResult = arrLayer[EType_Destination4].DrawTextCanvas(&CFLPointD(0, 0), L"Destination4 Image (Kernel Size 15x15)", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to draw text\n");
			break;
		}
