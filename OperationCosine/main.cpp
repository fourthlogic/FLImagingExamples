...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationCosine/Flower.flif")))
		{
		...
		if((eResult = cosine.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation cosine.");
			break;
		}
