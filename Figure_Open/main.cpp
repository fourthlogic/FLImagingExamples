...
int main()
{
	...
	do
	{
		...
		// Open result1 View 생성 // Create Open result1 view
		...
		// Open result2 View 생성 // Create Open result2 view
		...
		// Open result3 View 생성 // Create Open result3 view
		...
		layer[1].DrawTextCanvas(&CFLPoint<int32_t>(0, 0), L"Open Result1", YELLOW, BLACK, 30);
		layer[2].DrawTextCanvas(&CFLPoint<int32_t>(0, 0), L"Open Result2", YELLOW, BLACK, 30);
		layer[3].DrawTextCanvas(&CFLPoint<int32_t>(0, 0), L"Open Result3", YELLOW, BLACK, 30);
		...
		// Open 함수 실행 (KernelSize : 21, default kernel : Rectangle)
		// Open function execution (KernelSize : 21, default kernel : Rectangle)
		CFLFigureArray flfaResult1;
		int64_t i64KernelSize = 21;

		if((res = flrgSourceFig.Open(i64KernelSize, i64KernelSize, &flfaResult1)).IsFail())
		{
			ErrorPrint(res, "Failed to calculate.\n");
			break;
		}

		// Open 함수 실행 (KernelSize : 21, kernel shape : Circle)
		// Open function execution (KernelSize : 21, kernel shape : Circle)
		CFLFigureArray flfaResult2;

		if((res = flrgSourceFig.Open(i64KernelSize, i64KernelSize, &flfaResult2, EKernelShape_Circle)).IsFail())
		{
			ErrorPrint(res, "Failed to calculate.\n");
			break;
		}

		// Open 함수 실행 (Figure Kernel : 반지름이 10인 원)
		// Open function execution (Figure Kernel : Circle with radius 10)
		...
		if((res = flrgSourceFig.Open(&fleKernel, &flfaResult3)).IsFail())
		{
			ErrorPrint(res, "Failed to calculate.\n");
			break;
		}
