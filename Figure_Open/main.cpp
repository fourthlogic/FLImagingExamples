...
int main()
{
	...
	do
	{
		...
		// Open 함수 실행 (HalfSize : 10, default kernel : Rectangle)
		// Open function execution (HalfSize : 10, default kernel : Rectangle)
		CFLFigureArray flfaResult1;
		int64_t i64HalfSize = 10;

		if((res = flrgSourceFig.Open(i64HalfSize, i64HalfSize, &flfaResult1)).IsFail())
		{
			ErrorPrint(res, "Failed to calculate.\n");
			break;
		}

		// Open 함수 실행 (HalfSize : 10, kernel shape : Circle)
		// Open function execution (HalfSize : 10, kernel shape : Circle)
		CFLFigureArray flfaResult2;

		if((res = flrgSourceFig.Open(i64HalfSize, i64HalfSize, &flfaResult2, EKernelShape_Circle)).IsFail())
		{
			ErrorPrint(res, "Failed to calculate.\n");
			break;
		}
