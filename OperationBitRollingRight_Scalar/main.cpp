...
int main()
{
	...
	do
	{
		...
		if((res =arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(BitrollingRight 1)", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(res, L"Failed to draw text\n");
			break;
		}

		if((res =arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(BitrollingRight 7)", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(res, L"Failed to draw text\n");
			break;
		}
