...
int main()
{
	...
	do
	{
		...
		if((eResult =rollingRight.Execute()).IsFail())
		{
		...
		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if((eResult =rollingRight.Execute()))
		{
			ErrorPrint(eResult, L"Failed to execute operation rolling.");
			break;
		}
		...
		if((eResult =arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(BitrollingRight 1)", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to draw text\n");
			break;
		}

		if((eResult =arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(BitrollingRight 7)", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to draw text\n");
			break;
		}
