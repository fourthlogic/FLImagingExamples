...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionHarMean2Dto1D/Source.flif")))
		{
		...
		// Reduce Dimension HarMean 2D to 1D 객체 생성 // Create Reduce Dimension HarMean 2D to 1D object
		...
		reduceDimensionHarMean2Dto1D.SetDestinationImage(fliDestinationImageX);

		// 축소 차원 설정 // Set reduction dimension
		reduceDimensionHarMean2Dto1D.SetReductionDimension(CReduceDimensionHarMean2Dto1D::EReductionDimension_X);

		// 알고리즘 수행 // Execute the algorithm
		if((res = reduceDimensionHarMean2Dto1D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension HarMean 2D to 1D.\n");

			break;
		}

		// Destination 이미지 설정 // Set destination image
		reduceDimensionHarMean2Dto1D.SetDestinationImage(fliDestinationImageY);

		// 축소 차원 설정 // Set reduction dimension
		reduceDimensionHarMean2Dto1D.SetReductionDimension(CReduceDimensionHarMean2Dto1D::EReductionDimension_Y);
