...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationCosecant/Flower.flif")))
		{
		...
		if((eResult = Cosecant.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation Cosecant.");
			break;
		}
