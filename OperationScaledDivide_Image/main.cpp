...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationScaledDivide/Generator.flif")))
		{
		...
		if(IsFail(res = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationScaledDivide/Gradation_R2W.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		if(IsFail(res = arrLayer[EType_Source].DrawTextCanvas(&CFLPointD(0, 0), L"Source Image", YELLOW, BLACK, 18)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = arrLayer[EType_Operand].DrawTextCanvas(&CFLPointD(0, 0), L"Operand Image", YELLOW, BLACK, 18)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = arrLayer[EType_Destination].DrawTextCanvas(&CFLPointD(0, 0), L"Destination Image", YELLOW, BLACK, 18)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
