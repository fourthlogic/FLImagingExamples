...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Load image
		if(IsFail(eResult = fliImage.Load(L"../../ExampleImages/DataMatrix/Tray.flif")))
		{
		...
		for(int64_t i = 0; i < i64Results; ++i)
		{
			...
			if(IsFail(eResult = layer.DrawFigureImage(&flfaGridRegion, COLOR(rand() % 256, rand() % 256, rand() % 256), 2)))
			{
			...
			if(IsFail(eResult = layer.DrawTextImage(&flplPos, flstrDecoded, CYAN, BLACK, 12, false, flqdRegion.flpPoints[3].GetAngle(flqdRegion.flpPoints[2]))))
			{
