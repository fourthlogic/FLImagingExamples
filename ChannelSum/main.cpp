...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ChannelSum/Color.flif")))
		{
		...
		// Channel Sum 객체 생성 // Create Channel Sum object
		...
		if((res = channelSum.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Channel Sum.\n");
