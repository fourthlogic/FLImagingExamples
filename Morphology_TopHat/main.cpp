...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = arrFliImage[EType_Src].Load(L"../../ExampleImages/Morphology/Flower.flif")))
		{
		...
		// Morphology Top Hat 객체 생성 // Create Morphology Top Hat object
		...
		morphologyTopHat.SetKernelShape(EKernelShape_Circle);
		morphologyTopHat.SetKernel(7);
		// 연산 방법 설정
		morphologyTopHat.SetTransformMethod(CMorphologyTopHat::ETransformMethod_White);
		
		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if(IsFail(res = morphologyTopHat.Execute()))
		{
			ErrorPrint(res, "Failed to execute morphology tophat.");
			break;
		}
