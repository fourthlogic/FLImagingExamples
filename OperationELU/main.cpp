...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/OperationELU/Coord1D.flif")))
		{
		...
		// Operation ELU 객체 생성 // Create Operation ELU object
		COperationELU algObject;

		// 이미지 설정 // Set the images
		algObject.SetSourceImage(fliSourceImage);
		algObject.SetDestinationImage(fliDestinationImage0);
		algObject.SetOperationMode(COperationELU::EOperationMode_Forward);
		...
		algObject.SetOperationMode(COperationELU::EOperationMode_Backward);
