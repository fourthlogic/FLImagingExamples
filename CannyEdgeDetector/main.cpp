...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = fliISrcImage.Load(L"../../ExampleImages/CannyEdgeDetector/Circuit Board.flif")))
		{
		...
		if((eResult = CannyEdge.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute Canny Edge Detector.");
