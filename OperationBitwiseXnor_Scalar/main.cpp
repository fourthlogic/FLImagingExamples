...
int main()
{
	...
	do
	{
		...
		if((eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(BitwiseXnor 64)", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if((eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image(BitwiseXnor 128)", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
