...
int main()
{
	...
	do
	{
		...
		Dst1Layer0.DrawTextCanvas(TPoint<double>(0, 20), L"Index of Maximum Distance", CYAN, BLACK);
		...
		CFLScalar<int64_t> flvSrcDepth1 = (CFLScalar<int64_t>)flfaResultSrc2.GetAt(0);
		CFLScalar<int64_t> flvSrcDepth2 = (CFLScalar<int64_t>)flfaResultSrc2.GetAt(1);
		...
		flfaArraySrcDepth1.GetBoundaryRect(&flrBoundary2);

		Dst2Layer0.DrawFigureImage(flrBoundary2, LIGHTORANGE, 1);
		Dst2Layer0.DrawTextImage(flfaArraySrcDepth1.GetCenter(), CFLString<wchar_t>().Format(L"%lld", flvSrcDepth1.v), LIGHTORANGE, BLACK, 12, false, 0, EGUIViewImageTextAlignment_CENTER_CENTER);

		Dst2Layer0.DrawTextImage(flfaArraySrcDepth1.GetAt(flvSrcDepth2.v)->GetCenter(), CFLString<wchar_t>().Format(L"%lld", flvSrcDepth2.v), CYAN, BLACK, 12, false, 0, EGUIViewImageTextAlignment_CENTER_CENTER);
		...
		wprintf(L"Dst Result2 Index of Maximum distance\n");
		wprintf(L"Depth1 : %lld\n\n", flvDstDepth1.v);
