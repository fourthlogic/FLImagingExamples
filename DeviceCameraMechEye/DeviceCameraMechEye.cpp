...
int main()
{
	...
	do
	{
		...
		if(bAutoDetect)
		{
			...
			drReturn = camMechEye.GetAutoDetectCameraSerialNumbers(flarrSerialNumbers);
		...
		if(bAutoDetect)
			...
			drReturn = camMechEye.AutoDetectCamera(i32SelectDevice);
		else
			// 시리얼 번호를 설정합니다.
			camMechEye.SetSerialNumber(flsConnection);

		// 카메라를 초기화 합니다.
		drReturn = camMechEye.Initialize();
