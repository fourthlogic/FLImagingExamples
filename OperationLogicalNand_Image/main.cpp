...
int main()
{
	...
	do
	{
		...
		if((eResult = fliSourceImage.Load(L"../../ExampleImages/OperationLogicalNand/circle.flif")).IsFail())
		{
		...
		if((eResult = fliOperandImage.Load(L"../../ExampleImages/OperationLogicalNand/square.flif")).IsFail())
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}
		...
		logical.SetOperationSource(EOperationSource_Image);
