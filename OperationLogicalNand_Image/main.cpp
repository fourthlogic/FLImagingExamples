...
int main()
{
	...
	do
	{
		...
		if((res = fliSourceImage.Load(L"../../ExampleImages/OperationLogicalNand/circle.flif")).IsFail())
		{
		...
		if((res = fliOperandImage.Load(L"../../ExampleImages/OperationLogicalNand/square.flif")).IsFail())
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
