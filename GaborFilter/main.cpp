...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/GaborFilter/ComputerBoard.flif")))
		{
		...
		if(IsFail(eResult = arrViewImage[EType_Kernel].Create(612, 0, 1124, 512)))
		{
			ErrorPrint(eResult, "Failed to create the image view.\n");
			break;
		}
		...
		if(IsFail(eResult = arrViewImage[EType_Source].SynchronizeWindow(&arrViewImage[EType_Kernel])))
		{
		...
		if((eResult = gaborfilter.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute gabor filter.");
			break;
		}
		...
		if(IsFail(eResult = arrLayer[EType_Kernel].DrawTextCanvas(CFLPointD(0, 0), L"Kernel Image", YELLOW, BLACK, 30)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
