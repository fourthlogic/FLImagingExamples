...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = arrFliImage[EType_Src].Load(L"../../ExampleImages/Morphology/Moss.flif")))
		{
		...
		morphologyGradient.SetKernelShape(CMorphologyGradient::EKernelShape_Circle);
		morphologyGradient.SetKernel(3);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if(IsFail(eResult = morphologyGradient.Execute()))
		{
			ErrorPrint(eResult, "Failed to execute morphology gradient.");
			break;
		}
