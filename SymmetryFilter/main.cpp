...
int main()
{
	...
	do
	{
		...
		if((eResult = fliSourceImage.Load(L"../../ExampleImages/SymmetryFilter/Monkey.flif")).IsFail())
		{
		...
		if((eResult = fliDestinationImage.Load(L"../../ExampleImages/SymmetryFilter/Monkey.flif")).IsFail())
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}
		...
		// SymmetryFilter 객체 생성 // Create SymmetryFilter object
		...
		// Padding Method = DecreasingKernel 설정 // Set the Padding Method = DecreasingKernel
		symmetryFilter.SetPaddingMethod(EPaddingMethod_DecreasingKernel);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if((eResult = symmetryFilter.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute symmetry filter.\n");
