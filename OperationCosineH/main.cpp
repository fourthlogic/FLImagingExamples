...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationCosineH/Flower.flif")))
		{
		...
		if((eResult = CosineH.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation SineH.");
			break;
		}
