...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ChannelMedian/Source.flif")))
		{
		...
		// Channel Median 객체 생성 // Create Channel Median object
		...
		if((res = channelMedian.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Channel Median.\n");
