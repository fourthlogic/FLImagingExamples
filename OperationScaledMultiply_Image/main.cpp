...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationScaledMultiply/Generator.flif")))
		{
		...
		if(IsFail(res = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationScaledMultiply/Gradation_R2W.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		if(IsFail(res = arrLayer[EType_Operand].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Operand Image", YELLOW, BLACK, 20)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = arrLayer[EType_Destination].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination Image", YELLOW, BLACK, 20)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
