...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionModeMultiPageTo2D/Source.flif")))
		{
		...
		// Reduce Dimension Mode Multi Page To 2D 객체 생성 // Create Reduce Dimension Mode Multi Page To 2D object
		CReduceDimensionModeMultiPageTo2D reduceDimensionModeMultiPageTo2D;

		// Source 이미지 설정 // Set source image 
		reduceDimensionModeMultiPageTo2D.SetSourceImage(fliSourceImage);

		// Destination 이미지 설정 // Set destination image
		reduceDimensionModeMultiPageTo2D.SetDestinationImage(fliDestinationImage);

		// 알고리즘 수행 // Execute the algorithm
		if((res = reduceDimensionModeMultiPageTo2D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension Mode Multi Page To 2D.\n");
