...
int main()
{
	...
	do
	{
		...
		if((result = fliSrcImage.Load(L"../../ExampleImages/FreiFilter/Gear.flif")).IsFail())
		{
		...
		if((result = viewImageDst.Create(912, 0, 1424, 612)).IsFail())
		{
			ErrorPrint(result, L"Failed to create the image view.\n");
			break;
		}
		...
		if((result = viewImageDst.SetImagePtr(&fliDstImage)).IsFail())
		{
			ErrorPrint(result, L"Failed to set image object on the image view.\n");
			break;
		}
		...
		if((result = viewImageSrc.SynchronizePointOfView(&viewImageDst)).IsFail())
		{
		...
		if((result = viewImageSrc.SynchronizeWindow(&viewImageDst)).IsFail())
		{
		...
		CKirschFilter kirschFilter;

		// Source 이미지 설정 // Set source image
		kirschFilter.SetSourceImage(fliSrcImage);
		// Destination 이미지 설정 // Set destination image
		kirschFilter.SetDestinationImage(fliDstImage);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if((result = kirschFilter.Execute()).IsFail())
		{
			ErrorPrint(result, L"Failed to execute KirschFilter.");
			break;
		}

		// Destination 이미지가 새로 생성됨으로 Zoom fit 을 통해 디스플레이 되는 이미지 배율을 화면에 맞춰준다. // With the newly created Destination image, the image magnification displayed through Zoom fit is adjusted to the screen.
		if((result = viewImageDst.ZoomFit()).IsFail())
		{
		...
		if((result = layerDst.DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination Image", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}
