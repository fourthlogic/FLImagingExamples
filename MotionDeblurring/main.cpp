...
int main()
{
	...
	do
	{
		...
		if((eResult = fliSrcImage.Load(L"../../ExampleImages/MotionDeblurring/CircuitBlur.flif")).IsFail())
		{
		...
		// MotionDeblurring 객체 생성 // Create MotionDeblurring object
		CMotionDeblurring MotionDeblurring;

		// Source 이미지 설정 // Set the source image
		MotionDeblurring.SetSourceImage(fliSrcImage);
		// Destination 이미지 설정 // Set the destination image
		MotionDeblurring.SetDestinationImage(fliDstImage);

		// Auto Mode 설정 // Set Auto Mode
		MotionDeblurring.EnableAuto(true);
		// 임계값 설정 // Set Threshold
		MotionDeblurring.SetThreshold(0.41);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if((eResult = MotionDeblurring.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute MotionDeblurring.");
			break;
		}
