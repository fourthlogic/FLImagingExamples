...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionMeanMultiPageTo2D/Source.flif")))
		{
		...
		// Reduce Dimension Mean Multi Page To 2D 객체 생성 // Create Reduce Dimension Mean Multi Page To 2D object
		CReduceDimensionMeanMultiPageTo2D reduceDimensionMeanMultiPageTo2D;

		// Source 이미지 설정 // Set source image 
		reduceDimensionMeanMultiPageTo2D.SetSourceImage(fliSourceImage);

		// Destination 이미지 설정 // Set destination image
		reduceDimensionMeanMultiPageTo2D.SetDestinationImage(fliDestinationImage);

		// 알고리즘 수행 // Execute the algorithm
		if((res = reduceDimensionMeanMultiPageTo2D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension Mean Multi Page To 2D.\n");
