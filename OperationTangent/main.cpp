...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationTangent/Flower.flif")))
		{
		...
		if((eResult = tangent.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation tangent.");
			break;
		}
