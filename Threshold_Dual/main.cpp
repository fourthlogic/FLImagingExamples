...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image 
		if(IsFail(eResult = fliISrcImage.Load(L"../../ExampleImages/Threshold/Mountain.flif")))
		{
		...
		if(IsFail(eResult = viewImage2.Create(300 + 512, 0, 300 + 512 * 2, 384)))
		{
			ErrorPrint(eResult, "Failed to create the image view.\n");
			break;
		}
		...
		if(IsFail(eResult = viewImage2.SetImagePtr(&fliIDstImage)))
		{
			ErrorPrint(eResult, "Failed to set image object on the image view.\n");
			break;
		}
		...
		if(IsFail(eResult = viewImage1.SynchronizePointOfView(&viewImage2)))
		{
		...
		if(IsFail(eResult = viewImage1.SynchronizeWindow(&viewImage2)))
		{
		...
		threshold.SetThresholdMode(EThresholdMode_Dual_And);
		...
		mvThresholdValue1.PushBack(100);
		mvThresholdValue2.PushBack(168);

		// 임계값 설정 (다채널 경우 CMultiVar<double> 사용) // Set threshold value(Use CMultiVar<double> for multi-channel)
		threshold.SetThreshold(mvThresholdValue1, mvThresholdValue2);
		...
		threshold.SetLogicalCondition(mvCondition1, mvCondition2);

		// 알고리즘 수행 // Execute the algorithm
		if((eResult = threshold.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute Absolute Threshold.");
			break;
		}
		...
		if(IsFail(eResult = layer.DrawTextCanvas(&CFLPointD(0, 0), L"100 < threshold < 168", YELLOW, BLACK, 30, false, 0, EGUIViewImageTextAlignment_LEFT_TOP, L"", 1.f, 1.f, EGUIViewImageFontWeight_BOLD, false)))
		{
