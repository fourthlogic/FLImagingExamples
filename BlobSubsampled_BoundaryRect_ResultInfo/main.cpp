...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = blobSubsampled.Execute()))
		{
		...
		if(IsFail(eResult = blobSubsampled.Filter(CBlobSubsampled::EFilterItem_Area, 500, ELogicalCondition_LessEqual)))
		{
		...
		if(IsFail(eResult = blobSubsampled.GetResultBoundaryRects(flfaBoundaryRects)))
		{
		...
		if(IsFail(eResult = layer.DrawFigureImage(&flfaBoundaryRects, RED, 1, RED, EGUIViewImagePenStyle_Solid, 1, 0.25)))
		{
