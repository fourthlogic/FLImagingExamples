...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionCount2Dto1D/Source.flif")))
		{
		...
		reduceDimensionCount2Dto1D.SetDestinationImage(fliDestinationImageX);

		// 축소 차원 설정 // Set reduction dimension
		reduceDimensionCount2Dto1D.SetReductionDimension(CReduceDimensionCount2Dto1D::EReductionDimension_X);
		...
		// Destination 이미지 설정 // Set destination image
		reduceDimensionCount2Dto1D.SetDestinationImage(fliDestinationImageY);

		// 축소 차원 설정 // Set reduction dimension
		reduceDimensionCount2Dto1D.SetReductionDimension(CReduceDimensionCount2Dto1D::EReductionDimension_Y);
