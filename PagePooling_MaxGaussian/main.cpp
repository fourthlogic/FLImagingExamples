...
int main()
{
	...
	{
		// 이미지 로드 // Load image
		if((res = fliSourceImage.Load(L"../../ExampleImages/PagePooling/Multiple File_Max.flif")).IsFail())
		{
		...
		pagePooling.SetSamplingMethod(CPagePooling::ESamplingMethod_MaxGaussian);
		...
		if((res = layerIndex.DrawTextCanvas(&CFLPointD(0, 0), L"Index Image", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(res, L"Failed to draw text\n");
			break;
		}
