...
int main()
{
	...
	do
	{
		...
		layer[1].DrawTextCanvas(&CFLPoint<int32_t>(0, 0), L"Close Result1", YELLOW, BLACK, 30);
		layer[2].DrawTextCanvas(&CFLPoint<int32_t>(0, 0), L"Close Result2", YELLOW, BLACK, 30);
		layer[3].DrawTextCanvas(&CFLPoint<int32_t>(0, 0), L"Close Result3", YELLOW, BLACK, 30);
		...
		// Close 함수 실행 (HalfSize : 10, default kernel : Rectangle)
		// Close function execution (HalfSize : 10, default kernel : Rectangle)
		CFLFigureArray flfaResult1;
		int64_t i64HalfSize = 10;

		if((res = flrgSourceFig.Close(i64HalfSize, i64HalfSize, &flfaResult1)).IsFail())
		{
			ErrorPrint(res, "Failed to calculate.\n");
			break;
		}

		// Close 함수 실행 (HalfSize : 10, kernel shape : Circle)
		// Close function execution (HalfSize : 10, kernel shape : Circle)
		CFLFigureArray flfaResult2;

		if((res = flrgSourceFig.Close(i64HalfSize, i64HalfSize, &flfaResult2, EKernelShape_Circle)).IsFail())
		{
			ErrorPrint(res, "Failed to calculate.\n");
			break;
		}
		...
		if((res = flrgSourceFig.Close(&fleKernel, &flfaResult3)).IsFail())
		{
			ErrorPrint(res, "Failed to calculate.\n");
			break;
		}
