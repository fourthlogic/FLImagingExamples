...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = fliISrcImage.Load(L"../../ExampleImages/DericheEdgeDetector/Circuit Board.flif")))
		{
		...
		if((eResult = DericheEdge.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute Deriche Edge Detector.");
