...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination1 Image(ScaledDivide 192)", YELLOW, BLACK, 18)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination2 Image(ScaledDivide 512)", YELLOW, BLACK, 18)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
