...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/OperationPReLU/Coord1D.flif")))
		{
		...
		// Operation PReLU 객체 생성 // Create Operation PReLU object
		...
		algObject.SetOperationMode(COperationPReLU::EOperationMode_Forward);
		...
		algObject.SetOperationMode(COperationPReLU::EOperationMode_Backward);
