...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/Convolution/Binding.flif")))
		{
		...
		convolutionHighpass.SetKernelMethod(CConvolutionHighpass::EKernel_Highpass1);
 		...
 		if(IsFail(eResult = convolutionHighpass.Execute()))
 		{
 			ErrorPrint(eResult, "Failed to execute convolution Highpass.");
 			break;
 		}
		...
		if(IsFail(eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image (Highpass1)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if(IsFail(eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image (Highpass2)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
