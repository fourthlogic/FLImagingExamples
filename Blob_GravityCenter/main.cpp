...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = blob.Filter(CBlob::EFilterItem_BoundaryRectWidth, 20, ELogicalCondition_Less)))
		{
		...
		if(IsFail(eResult = blob.Filter(CBlob::EFilterItem_BoundaryRectHeight, 20, ELogicalCondition_Less)))
		{
			ErrorPrint(eResult, "Blob filtering algorithm error occurred.");
			break;
		}
		...
		{
			ErrorPrint(eResult, "Failed to get gravity center from the Blob object.");
			break;
		}
