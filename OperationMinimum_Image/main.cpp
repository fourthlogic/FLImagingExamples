...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationMinimum/Flower.flif")))
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(eResult = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationMinimum/Table.flif")))
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}
		...
		minimum.SetOperandImage(arrFliImage[EType_Operand]);
		// Destination 이미지 설정 // Set destination image 
		minimum.SetDestinationImage(arrFliImage[EType_Destination]);
		// 연산 방식 설정 // Set operation source
		minimum.SetOperationSource(EOperationSource_Image);

		// 알고리즘 수행 // Execute the algorithm
		if((eResult = minimum.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation minimum.");
			break;
		}
