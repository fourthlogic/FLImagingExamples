...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = blob.Filter(CBlobSubsampled::EFilterItem_BoundaryRectWidth, 20, ELogicalCondition_Less)))
		{
		...
		if(IsFail(res = blob.Filter(CBlobSubsampled::EFilterItem_BoundaryRectHeight, 20, ELogicalCondition_Less)))
		{
			ErrorPrint(res, "BlobSubsampled filtering algorithm error occurred.");
			break;
		}
