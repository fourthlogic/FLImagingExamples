...
int main()
{
	...
	{
		// 이미지 로드 // Load image
		if(IsFail(eResult = fliSourceImage.Load(L"../../ExampleImages/Perspective/calendar.flif")))
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(eResult = fliDestinationImage.Load(L"../../ExampleImages/Perspective/space.flif")))
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}
		...
		if(IsFail(eResult = viewImageSource.Create(400, 0, 912, 384)))
		{
		...
		if(IsFail(eResult = viewImageDestination.Create(912, 0,1424, 384)))
		{
			ErrorPrint(eResult, "Failed to create the image view.\n");
			break;
		}
		...
		if(IsFail(eResult = perspective.Execute()))
		{
			ErrorPrint(eResult, "Failed to execute perspective.\n");
			break;
		}
		...
		if(IsFail(eResult = layerSource.DrawFigureImage(&flqSourceProjection, LIME, 3)))
			ErrorPrint(eResult, "Failed to draw figure\n");

		// Destination Projection 영역이 어디인지 알기 위해 디스플레이한다. // Display to know where the Destination Projection area is.
		if(IsFail(eResult = layerDestination.DrawFigureImage(&flrDestinationProjection, LIME, 3)))
			ErrorPrint(eResult, "Failed to draw figure\n");

		// Destination ROI 영역이 어디인지 알기 위해 디스플레이한다. // Display to know where the Destination ROI area is.
		if(IsFail(eResult = layerDestination.DrawFigureImage(&flcDestinationROI, RED, 3)))
			ErrorPrint(eResult, "Failed to draw figure\n");
