...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/OperationSwish/Coord1D.flif")))
		{
		...
		// Operation Swish 객체 생성 // Create Operation Swish object
		COperationSwish algObject;

		// 이미지 설정 // Set the images
		algObject.SetSourceImage(fliSourceImage);
		algObject.SetDestinationImage(fliDestinationImage0);
		algObject.SetOperationMode(COperationSwish::EOperationMode_Forward);
		...
		algObject.SetOperationMode(COperationSwish::EOperationMode_Backward);
