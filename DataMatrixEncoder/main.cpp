...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = dataMatrixEncoder.Execute()))
		{
			ErrorPrint(res, "Failed to execute data matrix encoder.");
		...
		for(int64_t i = 0; i < i64Results; ++i)
		{
			...
			if(IsFail(res = layer.DrawTextImage(&flplPos, flstrDecoded, CYAN, BLACK, 20)))
			{
