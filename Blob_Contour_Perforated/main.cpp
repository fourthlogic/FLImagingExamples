...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Load image
		if(IsFail(eResult = fliImage.Load(L"../../ExampleImages/Blob/Perforated.flif")))
		{
...
// 		blob.SetThreshold(240, EThresholdIndex_Second);
		...
		if(IsFail(eResult = layer.DrawFigureImage(&flfaContours, RED, 1, RED, EGUIViewImagePenStyle_Solid, 1, .2f)))
		{
