...
int main()
{
	...
	do
	{
		...
		if((result = arrLayer[0].DrawTextCanvas(&CFLPointD(0, 0), L"Source Image", YELLOW, BLACK, 25)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination Image(Greater 128)", YELLOW, BLACK, 25)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}
