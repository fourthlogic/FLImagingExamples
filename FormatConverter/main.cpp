...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = fliSourceImage.Load(L"../../ExampleImages/FormatConverter/city_3.flif")))
		{
		...
		if(IsFail(eResult = formatConverter.SetSourceImage(fliSourceImage)))
		{
		...
		if(IsFail(eResult = formatConverter.SetDestinationImage(fliConvertedImage)))
		{
		...
		if(IsFail(eResult = formatConverter.SetPixelFormat(EPixelFormat_C1_U8)))
		{
