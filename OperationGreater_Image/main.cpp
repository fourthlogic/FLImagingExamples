...
int main()
{
	...
	do
	{
		...
		if((result = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationLesser/block.flif")).IsFail())
		{
		...
		if((result = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationLesser/road.flif")).IsFail())
		{
			ErrorPrint(result, L"Failed to load the image file.\n");
			break;
		}
		...
		greater.SetSourceImage(arrFliImage[EType_Source]);
		// Operand 이미지 설정 // Set operand image
		greater.SetOperandImage(arrFliImage[EType_Operand]);
		// Destination 이미지 설정 // Set destination image
		greater.SetDestinationImage(arrFliImage[EType_Destination]);
		// Image Operation 모드로 설정 // Set operation mode to image
		greater.SetOperationSource(EOperationSource_Image);

		// Source가 Scalar보다 클 경우 값 설정 // Set output value if source is greater than scalar
		greater.SetRangeValue(CMultiVar<double>(240));
		// Source가 Scalar보다 작거나 같을 경우 값 설정 // Set output value if source is less than or equal to scalar
		greater.SetOutOfRangeValue(CMultiVar<double>(0));
		...
		if((result = arrLayer[EType_Source].DrawTextCanvas(&CFLPointD(0, 0), L"Source Image", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[EType_Operand].DrawTextCanvas(&CFLPointD(0, 0), L"Operand Image", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[EType_Destination].DrawTextCanvas(&CFLPointD(0, 0), L"Destination Image", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}
