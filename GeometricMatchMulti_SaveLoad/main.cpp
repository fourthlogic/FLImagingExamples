...
int main()
{
	...
	do
	{
		...
		for(int64_t i64DataIdx = 0; i64DataIdx < 3; ++i64DataIdx)
		{
			...
			if(IsFail(eResult = FLGeometricMatchMultiSave.Learn(arrClassName[i64DataIdx])))
			{
		...
		if(IsFail(eResult = FLGeometricMatchMultiSave.Save(L"../../ExampleImages/Matching/Geometric Multi Learn File")))
		{
		...
		if(IsFail(eResult = FLGeometricMatchMultiLoad.Load(L"../../ExampleImages/Matching/Geometric Multi Learn File")))
		{
		...
		// 알고리즘 수행 // Execute the algorithm
		eResult = FLGeometricMatchMultiLoad.Execute();
