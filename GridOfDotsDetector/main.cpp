...
int main()
{
	...
	do
	{
		...
		{
			gridofDots.GetResultCenterPoints(i64PageIndex, i64BoardIndex, flaPoints);
			gridofDots.GetResultBoardRegion(i64PageIndex, i64BoardIndex, flqRegion);
			i64ResultRow = gridofDots.GetResultBoardRows(i64PageIndex, i64BoardIndex);
			i64ResultCol = gridofDots.GetResultBoardColumns(i64PageIndex, i64BoardIndex);
			f64AverageCellPitch = gridofDots.GetResultBoardAverageCellPitch(i64PageIndex, i64BoardIndex, 0);
			...
			if(IsFail(res = layer.DrawTextImage(flqRegion.flpPoints[0], CFLString<wchar_t>().Format(L"(%lld X %lld) Pitch [%.01lf]", i64ResultCol, i64ResultRow, f64AverageCellPitch), YELLOW, BLACK, (int32_t)(f64Width / 16), true, f64Angle, EGUIViewImageTextAlignment_LEFT_BOTTOM, nullptr, 1.f, 1.f, EGUIViewImageFontWeight_EXTRABOLD)))
			{
