...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/OperationMish/Coord1D.flif")))
		{
		...
		// Operation Mish 객체 생성 // Create Operation Mish object
		COperationMish algObject;

		// 이미지 설정 // Set the images
		algObject.SetSourceImage(fliSourceImage);
		algObject.SetDestinationImage(fliDestinationImage0);
		algObject.SetOperationMode(COperationMish::EOperationMode_Forward);
		...
		algObject.SetOperationMode(COperationMish::EOperationMode_Backward);
