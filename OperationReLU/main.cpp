...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/OperationReLU/Coord1D.flif")))
		{
		...
		// Operation ReLU 객체 생성 // Create Operation ReLU object
		...
		algObject.SetOperationMode(COperationReLU::EOperationMode_Forward);
		...
		algObject.SetOperationMode(COperationReLU::EOperationMode_Backward);
