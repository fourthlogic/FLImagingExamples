...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(Sub 30, 0, 0)", YELLOW, BLACK, 25)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if(IsFail(eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image(Sub 0, 0, 30)", YELLOW, BLACK, 25)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
