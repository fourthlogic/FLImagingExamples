...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationSubtract/House.flif")))
		{
		...
		subtract.SetDestinationImage(arrFliImage[EType_Destination1]);
		// 연산 방식 설정 // Set operation source
		subtract.SetOperationSource(EOperationSource_Scalar);
		// Scalar 값 설정 // Set Scalar value
		subtract.SetScalarValue(CMultiVar<double>(30, 0, 0));

		// 알고리즘 수행 // Execute the algorithm
		if((eResult = subtract.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation subtract.");
			break;
		}

		// Destination 이미지 설정 // Set destination image 
		subtract.SetDestinationImage(arrFliImage[EType_Destination2]);
		// 연산 방식 설정 // Set operation source
		subtract.SetOperationSource(EOperationSource_Scalar);
		// Scalar 값 설정 // Set Scalar value
		subtract.SetScalarValue(CMultiVar<double>(0, 0, 30));
		...
		if(IsFail(eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(Sub 30, 0, 0)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if(IsFail(eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image(Sub 0, 0, 30)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
