...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/Convolution/Moss.flif")))
		{
		...
		CFLRectL flrROI(200, 200, 500, 500);
 		...
 		// Convolution Laplacian 객체 생성 // Create Convolution Laplacian object
		...
		convolutionLaplacian.SetKernelMethod(CConvolutionLaplacian::EKernel_LaplacianX);
 		...
 		if(IsFail(eResult = convolutionLaplacian.Execute()))
 		{
 			ErrorPrint(eResult, "Failed to execute convolution Laplacian.");
 			break;
 		}
		...
		if(IsFail(eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image (LaplacianX)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if(IsFail(eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image (LaplacianY)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if(IsFail(eResult = arrLayer[EType_Destination3].DrawTextCanvas(&CFLPointD(0, 0), L"Destination3 Image (Laplacian4)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if(IsFail(eResult = arrLayer[EType_Destination4].DrawTextCanvas(&CFLPointD(0, 0), L"Destination4 Image (Laplacian8)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
