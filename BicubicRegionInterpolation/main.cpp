...
int main()
{
	...
	do
	{
		...
		{
			ErrorPrint(eResult, "Failed to execute Bicubic Region Interpolation.");
			break;
		}
		...
		if(IsFail(eResult = arrLayer[EType_Destination].DrawTextCanvas(&CFLPointD(0, 0), L"Destination Image (Bicubic)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
