...
int main()
{
	...
	do
	{
		...
		if((res = fliSourceImage.Load(L"../../ExampleImages/OperationBitwiseNor/gradation.flif")).IsFail())
		{
		...
		// Operation BitwiseNor 객체 생성 // Create Operation BitwiseNor object
		COperationBitwiseNor bitnor;

		// Source 이미지 설정 // Set the source image
		bitnor.SetSourceImage(fliSourceDestinationImage);
		...
		bitnor.SetOperationSource(EOperationSource_Scalar);

		// 스칼라 값 지정 // Set the Scalar value
		bitnor.SetScalarValue(CMultiVar<double>(111));
