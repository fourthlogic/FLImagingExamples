...
int main()
{
	...
	do
	{
		...
		if((eResult = fliSourceImage.Load(L"../../ExampleImages/OperationBitwiseNor/gradation.flif")).IsFail())
		{
		...
		if((eResult = bitnor.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation bitwise nor.");
			break;
		}
