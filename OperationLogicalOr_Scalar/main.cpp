...
int main()
{
	...
	do
	{
		...
		if((eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationLogicalOr/Space.flif")).IsFail())
		{
		...
		if((eResult = logicalOr.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation logicalOr.");
			break;
		}
		...
		if((eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(LogicalOr 0)", YELLOW, BLACK, 25)).IsFail())
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if((eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image(LogicalOr 255)", YELLOW, BLACK, 25)).IsFail())
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
