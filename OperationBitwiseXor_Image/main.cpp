...
int main()
{
	...
	do
	{
		...
		if((res = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationBitwiseXor/Cat.flif")).IsFail())
		{
		...
		if((res = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationBitwiseXor/HangangRiver.flif")).IsFail())
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		bitwiseXor.SetOperandImage(arrFliImage[EType_Operand]);
		// Destination 이미지 설정 // Set the destination image
		bitwiseXor.SetDestinationImage(arrFliImage[EType_Destination]);
		// Image Operation 소스로 설정 // Set Operation Source to image
		bitwiseXor.SetOperationSource(EOperationSource_Image);
		...
		if((res = arrLayer[EType_Operand].DrawTextCanvas(&CFLPointD(0, 0), L"Operand Image", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if((res = arrLayer[EType_Destination].DrawTextCanvas(&CFLPointD(0, 0), L"Destination Image", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
