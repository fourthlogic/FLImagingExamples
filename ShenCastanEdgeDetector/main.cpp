...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = fliISrcImage.Load(L"../../ExampleImages/ShenCastanEdgeDetector_Img/Gear.flif")))
		{
		...
		if((eResult = ShenCastanEdge.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute ShenCastan Edge Detector.");
