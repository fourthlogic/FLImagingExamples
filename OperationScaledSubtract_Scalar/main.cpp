...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = ScaledSubtract.Execute()))
		{
			ErrorPrint(eResult, "Failed to execute operation ScaledSubtract.");
			break;
		}
		...
		if(IsFail(eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(ScaledSubtract 128)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if(IsFail(eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image(ScaledSubtract 430)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
