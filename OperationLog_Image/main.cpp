...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationLog/gradation_Horizontal.flif")))
		{
		...
		if(IsFail(res = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationLog/gradation_Vertical.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		log.SetOperandImage(arrFliImage[EType_Operand]);
		// Destination 이미지 설정 // Set the destination image
		log.SetDestinationImage(arrFliImage[EType_Destination]);
		// Image Operation 모드로 설정 // Set operation mode to image
		log.SetOperationSource(EOperationSource_Image);
