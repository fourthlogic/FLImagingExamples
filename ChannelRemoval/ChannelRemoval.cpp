...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ChannelRemoval/Color.flif")))
		{
		...
		CChannelRemoval channelRemoval;

		// Source 이미지 설정 // Set source image 
		channelRemoval.SetSourceImage(fliSourceImage);

		// Destination 이미지 설정 // Set destination image
		channelRemoval.SetDestinationImage(fliDestinationImage);

		// 삭제할 채널 설정 // Set channels to remove
		channelRemoval.SetRemovalChannels(EChannelSelection_Channel_0);
