...
int main()
{
	...
	do
	{
		...
		// Blob 결과의 Min, Max 를 얻어오기 위한 Statistics 객체 선언
		...
		// 각 Coutour에 맞는 Min, Max를 얻어오는 코드
		for(int64_t i = 0; i < i64ResultCount; ++i)
		{
			...
			strMin.Format(L"No. %d Min : ", i);
			strMin.AppendFormat(L"%lf", mvMin.GetAt(0));
			wprintf_s(L"%s\n", strMin.GetString());
			...
			strMax.Format(L"No. %d Max : ", i);
			strMax.AppendFormat(L"%lf", mvMax.GetAt(0));
			wprintf_s(L"%s\n", strMax.GetString());
		}
