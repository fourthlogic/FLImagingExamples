...
int main()
{
	...
	do
	{
		...
		CMultiFocusDerivativeBased3D algMultiFocusDerivativeBased3D;
		...
		algMultiFocusDerivativeBased3D.SetDestinationImage(fliDstImage);
		// 결과 Texture 이미지 설정 // Set the res texture image
		algMultiFocusDerivativeBased3D.SetResultTextureImage(&fliTxtImage);
		...
		algMultiFocusDerivativeBased3D.SetFilter(CMultiFocusDerivativeBased3D::EFilter_Guided);
		...
		CFL3DObjectHeightMap* pFl3DOHM = dynamic_cast<CFL3DObjectHeightMap*>(algMultiFocusDerivativeBased3D.GetDestinationObject());
