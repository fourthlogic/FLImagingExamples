...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionHarMeanMultiPageTo2D/Source.flif")))
		{
		...
		// Reduce Dimension HarMean Multi Page To 2D 객체 생성 // Create Reduce Dimension HarMean Multi Page To 2D object
		...
		if((res = reduceDimensionHarMeanMultiPageTo2D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension HarMean Multi Page To 2D.\n");
