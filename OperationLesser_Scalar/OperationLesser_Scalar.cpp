...
int main()
{
	...
	do
	{
		...
		if((result = arrLayer[0].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Source Image", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[1].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination Image(Lesser 128)", YELLOW, BLACK, 30)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}
