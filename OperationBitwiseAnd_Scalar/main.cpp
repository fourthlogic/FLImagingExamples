...
int main()
{
	...
	do
	{
		...
		if((res = fliSourceImage.Load(L"../../ExampleImages/OperationBitwiseAnd/gradation.flif")).IsFail())
		{
		...
		bitand.SetSourceImage(fliSourceDestinationImage);
		...
		bitand.SetOperationSource(EOperationSource_Scalar);

		// 스칼라 값 지정 // Set the Scalar value
		bitand.SetScalarValue(CMultiVar<double>(111));
