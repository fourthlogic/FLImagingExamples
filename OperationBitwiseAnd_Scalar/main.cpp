...
int main()
{
	...
	do
	{
		...
		if((eResult = fliSourceImage.Load(L"../../ExampleImages/OperationBitwiseAnd/gradation.flif")).IsFail())
		{
		...
		if((eResult = bitand.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation bitwise and.");
			break;
		}
