...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionMedianMultiPageTo2D/Source.flif")))
		{
		...
		// Reduce Dimension Median Multi Page To 2D 객체 생성 // Create Reduce Dimension Median Multi Page To 2D object
		CReduceDimensionMedianMultiPageTo2D reduceDimensionMedianMultiPageTo2D;

		// Source 이미지 설정 // Set source image 
		reduceDimensionMedianMultiPageTo2D.SetSourceImage(fliSourceImage);

		// Destination 이미지 설정 // Set destination image
		reduceDimensionMedianMultiPageTo2D.SetDestinationImage(fliDestinationImage);

		// 알고리즘 수행 // Execute the algorithm
		if((res = reduceDimensionMedianMultiPageTo2D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension Median Multi Page To 2D.\n");
