...
int main()
{
	...
	do
	{
		...
		if(IsFail(arrLayer[EType_Source].DrawTextCanvas(&CFLPointD(0, 0), L"Source Image", YELLOW, BLACK, 30)))
		{
			printf("Failed to draw text\n");
			break;
		}

		if(IsFail(arrLayer[EType_Destination].DrawTextCanvas(&CFLPointD(0, 0), L"Destination Image", YELLOW, BLACK, 30)))
		{
			printf("Failed to draw text\n");
			break;
		}
