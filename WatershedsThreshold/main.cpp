...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = fliISrcImage.Load(L"../../ExampleImages/AnisotropicDiffusion/Source.flif")))
		{
		...
		{
			ErrorPrint(eResult, "Failed to execute Watersheds Threshold.");
			break;
		}
		...
		if(IsFail(eResult = layer.DrawTextCanvas(&CFLPointD(0, 0), L"50 < threshold", YELLOW, BLACK, 30, false, 0, EGUIViewImageTextAlignment_LEFT_TOP, L"", 1.f, 1.f, EGUIViewImageFontWeight_BOLD, false)))
		{
