...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationAtan2/Sky.flif")))
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(eResult = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationAtan2/Flower.flif")))
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}
		...
		atan2.SetOperandImage(arrFliImage[EType_Operand]);
		// Destination 이미지 설정 // Set destination image 
		atan2.SetDestinationImage(arrFliImage[EType_Destination]);
		// 연산 방식 설정 // Set operation source
		atan2.SetOperationSource(EOperationSource_Image);
