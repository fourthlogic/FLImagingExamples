...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/OperationSigmoid/Coord1D.flif")))
		{
		...
		// Operation Sigmoid 객체 생성 // Create Operation Sigmoid object
		...
		algObject.SetOperationMode(COperationSigmoid::EOperationMode_Forward);
		...
		algObject.SetOperationMode(COperationSigmoid::EOperationMode_Backward);
