...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliISrcImage.Load(L"../../ExampleImages/Threshold/Checker Board_1Ch.flif")))
		{
		...
		ht.SetLowThreshold(CMultiVar<double>(110));

		// High Threshold 설정 // Set High Threshold
		ht.SetHighThreshold(CMultiVar<double>(190));
