...
int main()
{
	...
	do
	{
		...
		if((eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/Threshold/Mountain.flif")).IsFail())
		{
		...
		if((eResult = AutoThreshold.Execute()).IsFail())
		{
		...
		for(int64_t i = 0; i < i64IndexCount; ++i)
		{
			...
			for(int j = 0; j < i64ChannelCount; ++j)
			{
				...
				if((eResult = (AutoThreshold.GetResultBatch(i, j, flaResult))).IsFail())
				{
					ErrorPrint(eResult, L"No Result.");
					break;
				}
		...
		if((eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image (Sigma 2)", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to draw text\n");
			break;
		}

		if((eResult = arrLayer[EType_Destination1].DrawTextCanvas(flpThreshold, flaThresholdStr[0], YELLOW, BLACK, 16)).IsFail())
		{
			ErrorPrint(eResult, "Failed to draw text.\n");
			break;
		}

		if((eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image (Sigma 4)", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to draw text\n");
			break;
		}

		if((eResult = arrLayer[EType_Destination2].DrawTextCanvas(flpThreshold, flaThresholdStr2[0], YELLOW, BLACK, 16)).IsFail())
		{
			ErrorPrint(eResult, "Failed to draw text.\n");
			break;
		}
