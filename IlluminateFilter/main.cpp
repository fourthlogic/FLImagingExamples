...
int main()
{
	...
	do
	{
		...
		if((result = arrFliImage[EType_Source].Load(L"../../ExampleImages/Illuminate/Basement.flif")).IsFail())
		{
		...
		if((result = arrViewImage[EType_Destination1].Create(612, 0,1124, 512)).IsFail())
		{
			ErrorPrint(result, L"Failed to create the image view.\n");
			break;
		}
		...
		if((result = arrViewImage[EType_Destination2].Create(1124, 0,1636, 512)).IsFail())
		{
			ErrorPrint(result, L"Failed to create the image view.\n");
			break;
		}
		...
		// CIlluminateFilter 객체 생성 // Create CIlluminateFilter object
		CIlluminateFilter illuminateFilter;
		// Source 이미지 설정 // Set source image
		illuminateFilter.SetSourceImage(arrFliImage[EType_Source]);
		// Destination 이미지 설정 // Set destination image
		illuminateFilter.SetDestinationImage(arrFliImage[EType_Destination1]);
		// 파라미터 값 설정 // Set parameter value
		illuminateFilter.SetKernel(41);
		illuminateFilter.SetFactor(0.55f);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if((result = illuminateFilter.Execute()).IsFail())
		{
			ErrorPrint(result, L"Failed to execute Illuminate Filter.");
			break;
		}

		// Destination 이미지 설정 // Set destination image
		illuminateFilter.SetDestinationImage(arrFliImage[EType_Destination2]);
		// 파라미터 값 설정 // Set parameter value
		...
		if((result = arrLayer[EType_Source].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Source Image", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPoint<double>(0, 0), L"IlluminateFilter Size : 41x41 Factor: 0.55", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPoint<double>(0, 0), L"IlluminateFilter Size : 151x151 Factor: 0.80", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}
