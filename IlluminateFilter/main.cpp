...
int main()
{
	...
	do
	{
		...
		if((result = arrLayer[EType_Source].DrawTextCanvas(&CFLPointD(0, 0), L"Source Image", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"IlluminateFilter Size : 41x41 Factor: 0.55", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"IlluminateFilter Size : 151x151 Factor: 0.80", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}
