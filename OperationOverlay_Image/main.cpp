...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationOverlay/Sea.flif")))
		{
		...
		if(IsFail(eResult = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationOverlay/Moon.flif")))
		{
			ErrorPrint(eResult, "Failed to load the image file.\n");
			break;
		}
		...
		if(IsFail(eResult = overlay.Execute()))
		{
			ErrorPrint(eResult, "Failed to execute operation overlay.");
			break;
		}
