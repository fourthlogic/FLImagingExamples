...
int main()
{
	...
	do
	{
		...
		if((eResult = floSourceObject.Load(L"../../ExampleImages/VoxelGrid3D/VoxelGrid3DExample.ply")).IsFail())
		{
		...
		voxelGrid3D.SetSamplingDistance(0.03f);

		// 샘플링 방법 설정 // Set the sampling method
		voxelGrid3D.SetSamplingMethod(CVoxelGrid3D::ESamplingMethod_Mean);
		...
		{
			ErrorPrint(eResult, L"Failed to execute Voxel Grid 3D.");
			break;
		}
		...
		if((eResult = view3DDst.PushObject((CFL3DObject)voxelGrid3D.GetDestinationObject())).IsFail())
		{
