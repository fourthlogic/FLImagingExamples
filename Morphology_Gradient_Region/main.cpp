...
int main()
{
	...
	do
	{
		...
		// Morphology Gradient 객체 생성 // Create Morphology Gradient object
		CMorphologyGradient morphologyGradient;
		// ROI 범위 설정
		CFLRectL flrROI(100, 190, 360, 420);
		// 처리할 이미지 설정
		morphologyGradient.SetSourceImage(arrFliImage[EType_Src]);
		// 처리할 ROI 설정
		morphologyGradient.SetSourceROI(flrROI);

		// Destination 이미지 설정 // Set the destination image
		morphologyGradient.SetDestinationImage(arrFliImage[EType_Dst]);
		// Destination ROI 설정
		...
		morphologyGradient.SetKernel(&flfKernal);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if(IsFail(eResult = morphologyGradient.Execute()))
		{
			ErrorPrint(eResult, "Failed to execute morphology erode.");
			break;
		}
