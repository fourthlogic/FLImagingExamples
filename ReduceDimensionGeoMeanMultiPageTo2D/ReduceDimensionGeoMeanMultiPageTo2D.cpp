...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionGeoMeanMultiPageTo2D/Source.flif")))
		{
		...
		// Reduce Dimension GeoMean Multi Page To 2D 객체 생성 // Create Reduce Dimension GeoMean Multi Page To 2D object
		CReduceDimensionGeoMeanMultiPageTo2D reduceDimensionGeoMeanMultiPageTo2D;

		// Source 이미지 설정 // Set source image 
		reduceDimensionGeoMeanMultiPageTo2D.SetSourceImage(fliSourceImage);

		// Destination 이미지 설정 // Set destination image
		reduceDimensionGeoMeanMultiPageTo2D.SetDestinationImage(fliDestinationImage);

		// 알고리즘 수행 // Execute the algorithm
		if((res = reduceDimensionGeoMeanMultiPageTo2D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension GeoMean Multi Page To 2D.\n");
