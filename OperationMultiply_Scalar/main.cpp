...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationMultiply/Table.flif")))
		{
		...
		if(IsFail(res = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination1 Image(Multiply 1.5)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination2 Image(Multiply 2)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
