...
int main()
{
	...
	do
	{
		...
		if((result = arrFliImage[0].Load(L"../../ExampleImages/IntensityClamping/Color.flif")).IsFail())
		{
		...
		// intensityClamping Min&Max Intensity 값 설정 // Set min&max Intensity value of intensityClamping
		intensityClamping.SetIntensity(CMultiVar<double>(150, 150, 150), CMultiVar<double>(200, 200, 200));

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if((result = intensityClamping.Execute()).IsFail())
		{
			ErrorPrint(result, L"Failed to execute operation intensityClamping.");
