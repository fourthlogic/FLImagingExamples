...
int main()
{
	...
	do
	{
		...
		if((result = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Filter Size : 5x5", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}

		if((result = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Filter Size : 25x25", YELLOW, BLACK, 20)).IsFail())
		{
			ErrorPrint(result, L"Failed to draw text\n");
			break;
		}
