...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionGeoMean2Dto1D/Source.flif")))
		{
		...
		// Reduce Dimension GeoMean 2D to 1D 객체 생성 // Create Reduce Dimension GeoMean 2D to 1D object
		CReduceDimensionGeoMean2Dto1D reduceDimensionGeoMean2Dto1D;

		// Source 이미지 설정 // Set source image 
		reduceDimensionGeoMean2Dto1D.SetSourceImage(fliSourceImage);

		// Destination 이미지 설정 // Set destination image
		reduceDimensionGeoMean2Dto1D.SetDestinationImage(fliDestinationImageX);

		// 축소 차원 설정 // Set reduction dimension
		reduceDimensionGeoMean2Dto1D.SetReductionDimension(CReduceDimensionGeoMean2Dto1D::EReductionDimension_X);

		// 알고리즘 수행 // Execute the algorithm
		if((res = reduceDimensionGeoMean2Dto1D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension GeoMean 2D to 1D.\n");

			break;
		}

		// Destination 이미지 설정 // Set destination image
		reduceDimensionGeoMean2Dto1D.SetDestinationImage(fliDestinationImageY);

		// 축소 차원 설정 // Set reduction dimension
		reduceDimensionGeoMean2Dto1D.SetReductionDimension(CReduceDimensionGeoMean2Dto1D::EReductionDimension_Y);
