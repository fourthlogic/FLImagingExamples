...
int main()
{
	...
	do
	{
		...
		if((eResult = fliSourceImage.Load(L"../../ExampleImages/PagePixelPicker/MultiFile_Normal.flif")).IsFail())
		{
		...
		// Index 이미지 로드
		if((eResult = fliIndexImage.Load(L"../../ExampleImages/PagePixelPicker/IndexMap.flif")).IsFail())
		{
			ErrorPrint(eResult, L"Failed to load the image file.\n");
			break;
		}
		...
		if((eResult = viewImageSource.Create(400, 0, 900, 500)).IsFail())
		{
		...
		// Index 이미지 뷰 생성
		if((eResult = viewImageIndex.Create(900, 0, 1400, 500)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to create the image view.\n");
			break;
		}
		...
		if((eResult = viewImageDestination.Create(1400, 0, 1900, 500)).IsFail())
		{
			ErrorPrint(eResult, L"Failed to create the image view.\n");
			break;
		}
		...
		if((eResult = pagePixelPicker.Execute()).IsFail())
		{
			ErrorPrint(eResult, L"Failed to execute Page Pixel Picker.\n");
