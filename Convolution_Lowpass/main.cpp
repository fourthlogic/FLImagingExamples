...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/Convolution/Bridge.flif")))
		{
		...
		convolutionLowpass.SetKernelMethod(CConvolutionLowpass::EKernel_Lowpass1);
 		...
 		if(IsFail(eResult = convolutionLowpass.Execute()))
 		{
		...
		if(IsFail(eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image (Lowpass1)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if(IsFail(eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image (Lowpass2)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
