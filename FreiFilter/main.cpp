...
int main()
{
	...
	do
	{
		...
		if((eResult =fliSrcImage.Load(L"../../ExampleImages/FreiFilter/Gear.flif")).IsFail())
		{
		...
		if((eResult =FreiFilter.Execute()).IsFail())
		{
			ErrorPrint(eResult, L"Failed to execute FreiFilter.");
			break;
		}
