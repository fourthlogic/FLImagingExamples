...
int main()
{
	...
	do
	{
		...
		// Result Type 설정
		blob.SetResultType(CBlobSubsampled::EBlobResultType_Contour);
		...
		if(IsFail(eResult = blob.Filter(CBlobSubsampled::EFilterItem_Area, 50, ELogicalCondition_Less)))
		{
		...
		if(IsFail(eResult = blob.Filter(CBlobSubsampled::EFilterItem_Circularity, 0.7, ELogicalCondition_Less)))
		{
			ErrorPrint(eResult, "BlobSubsampled filtering algorithm error occurred.");
			break;
		}
