...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = blob.Filter(CBlobSubsampled::EFilterItem_Area, 50, ELogicalCondition_Less)))
		{
		...
		if(IsFail(res = blob.Filter(CBlobSubsampled::EFilterItem_Circularity, 0.7, ELogicalCondition_Less)))
		{
			ErrorPrint(res, "BlobSubsampled filtering algorithm error occurred.");
			break;
		}
