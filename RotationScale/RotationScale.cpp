...
int main()
{
	...
	do
	{
		...
		rotationScale.SetSourceImage(arrFliImage[EType_Dst1]);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if(IsFail(res = rotationScale.Execute()))
		{
		...
		rotationScale.SetResizeMethod(EResizeMethod_Resize);
		// Source Image 설정 // Set Source Image
		rotationScale.SetSourceImage(arrFliImage[EType_Dst2]);
		...
		rotationScale.SetResizeMethod(EResizeMethod_Resize);
		// Source Image 설정 // Set Source Image
		rotationScale.SetSourceImage(arrFliImage[EType_Dst3]);
