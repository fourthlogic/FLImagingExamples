...
int main()
{
	...
	do
	{
		...
		if((res = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination1 Image (Distribution: Uniform)", YELLOW, BLACK, 15)).IsFail())
		{
			ErrorPrint(res, L"Failed to draw text\n");
			break;
		}

		if((res = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination2 Image (Distribution: Gaussian)", YELLOW, BLACK, 15)).IsFail())
		{
			ErrorPrint(res, L"Failed to draw text\n");
			break;
		}
