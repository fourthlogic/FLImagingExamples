...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/ReduceDimensionMinMultiPageTo2D/Source.flif")))
		{
		...
		// Reduce Dimension Min Multi Page To 2D 객체 생성 // Create Reduce Dimension Min Multi Page To 2D object
		CReduceDimensionMinMultiPageTo2D reduceDimensionMinMultiPageTo2D;

		// Source 이미지 설정 // Set source image 
		reduceDimensionMinMultiPageTo2D.SetSourceImage(fliSourceImage);

		// Destination 이미지 설정 // Set destination image
		reduceDimensionMinMultiPageTo2D.SetDestinationImage(fliDestinationImage);

		// 알고리즘 수행 // Execute the algorithm
		if((res = reduceDimensionMinMultiPageTo2D.Execute()).IsFail())
		{
			ErrorPrint(res, "Failed to execute Reduce Dimension Min Multi Page To 2D.\n");
