...
int main()
{
	...
	do
	{
		...
		if(IsFail(eResult = arrFliImage[EType_Src].Load(L"../../ExampleImages/Morphology/CityNight.flif")))
		{
		...
		//연산에 사용되는 Region 출력 // Displaty RegionShape
		int32_t i32X = EType_Region % 2;
		int32_t i32Y = EType_Region / 2;

		// 이미지 뷰 생성 // Create image view
		if(IsFail(eResult = arrViewImage[EType_Region].Create(i32X * 400 + 400, i32Y * 400, i32X * 400 + 400 + 200, i32Y * 400 + 200)))
		{
			ErrorPrint(eResult, "Failed to create the image view.\n");
			bError = true;
		}
		...
		// 이미지 뷰에 이미지를 디스플레이 // Display an image in an image view
		if(IsFail(eResult = arrViewImage[EType_Region].SetImagePtr(&arrFliImage[EType_Region])))
		{
			ErrorPrint(eResult, "Failed to set image object on the image view.\n");
			bError = true;
		}
	    ...
	    // Morphology Close 객체 생성 // Create Morphology Close object
		CMorphologyClose morphologyClose;
		// ROI 범위 설정
		CFLRectL flrROI(100, 190, 360, 420);
		// 처리할 이미지 설정
		morphologyClose.SetSourceImage(arrFliImage[EType_Src]);
		// 처리할 ROI 설정
		morphologyClose.SetSourceROI(flrROI);

		// Destination 이미지 설정 // Set the destination image
		morphologyClose.SetDestinationImage(arrFliImage[EType_Dst]);
		// Destination ROI 설정
		morphologyClose.SetDestinationROI(flrROI);
		...
		morphologyClose.SetKernel(&flfKernal);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if(IsFail(eResult = morphologyClose.Execute()))
		{
			ErrorPrint(eResult, "Failed to execute morphology close.");
			break;
		}
		...
		if(IsFail(eResult = arrLayer[EType_Region].DrawFigureImage(&flfKernal, LIME)))
			ErrorPrint(eResult, "Failed to draw figure\n");
		...
		if(IsFail(eResult = arrLayer[EType_Region].DrawTextCanvas(&CFLPointD(0, 0), L"Region Shape", YELLOW, BLACK, 20)))
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
