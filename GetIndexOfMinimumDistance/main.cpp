...
int main()
{
	...
	do
	{
		...
		if((eResult = flpaSource1.GetIndexOfMinimumDistance(flcDestination1, &flfaResultSrc1)).IsFail())
		{
		...
		if((eResult = flfaSource2.GetIndexOfMinimumDistance(flfaDestination2, &flfaResultSrc2, true, true, &flfaResultDst2)).IsFail())
		{
			ErrorPrint(eResult, "Failed to process.\n");
			break;
		}
