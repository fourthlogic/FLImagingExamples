...
int main()
{
	...
	do
	{
		...
		guidedFilter.SetOperationSource(EOperationSource_Image);

		// filter mask radius = 2 설정
		...
		if((eResult = guidedFilter.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute guided filter.\n");
