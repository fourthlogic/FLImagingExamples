...
int main()
{
	...
	do
	{
		...
		if((eResult = fliSrcImage.Load(L"../../ExampleImages/ShockFilter/ShockFilter.flif")).IsFail())
		{
		...
		if((eResult = ShockFilter.Execute()).IsFail())
		{
			ErrorPrint(eResult, L"Failed to execute ShockFilter.");
			break;
		}
