...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(eResult = fliImage.Load(L"../../ExampleImages/GridOfCrossDetector/GridOfCross.flif")))
		{
		...
		CGridOfCrossDetector gridofCross;

		// 처리할 이미지 설정 // set the image to process
		gridofCross.SetSourceImage(fliImage);

		// Grid Of Cross Detector 수행 // Execute Grid Of Cross Detector
		if(IsFail(eResult = gridofCross.Execute()))
		{
			ErrorPrint(eResult, "Failed to execute Grid Of Cross Detector.\n");
			break;
		}
		...
		gridofCross.GetResultCenterPoints(flaPoints);
		gridofCross.GetResultForRegion(flqRegion);
