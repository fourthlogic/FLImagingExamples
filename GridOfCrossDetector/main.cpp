...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = fliImage.Load(L"../../ExampleImages/GridOfCrossDetector/GridOfCross.flif")))
		{
		...
		if(IsFail(res = gridofCross.Execute()))
		{
		...
		int64_t i64BoardCount = gridofCross.GetResultBoardCount(i64PageIndex);

		for(int64_t i64BoardIndex = 0; i64BoardIndex < i64BoardCount; i64BoardIndex++)
		{
			gridofCross.GetResultCenterPoints(i64PageIndex, i64BoardIndex, flaPoints);
