...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = layerSource.DrawTextCanvas(&CFLPointD(0, 0), L"Source Image (BGR)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = layerConverted.DrawTextCanvas(&CFLPointD(0, 0), L"Converted Image (ARGB)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
