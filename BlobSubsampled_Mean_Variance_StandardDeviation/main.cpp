...
int main()
{
	...
	do
	{
		...
		for(int64_t i = 0; i < i64ResultCount; ++i)
		{
			...
			if(IsFail(res = imgStatistics.GetMean(mvMean)))
			{
			...
			if(IsFail(res = imgStatistics.GetVariance(mvVariance)))
			{
			...
			if(IsFail(res = imgStatistics.GetStandardDeviation(mvStandardDeviation)))
			{
