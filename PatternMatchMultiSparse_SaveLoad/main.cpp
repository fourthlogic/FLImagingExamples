...
int main()
{
	...
	do
	{
		...
		for(int64_t i64DataIdx = 0; i64DataIdx < 2; ++i64DataIdx)
		{
			...
			if(IsFail(eResult = FLPatternMatchMultiSparseSave.Learn(arrClassName[i64DataIdx])))
			{
		...
		if(IsFail(eResult = FLPatternMatchMultiSparseSave.Save(L"../../ExampleImages/Matching/Pattern Multi Learn File")))
		{
		...
		if(IsFail(eResult = FLPatternMatchMultiSparseLoad.Load(L"../../ExampleImages/Matching/Pattern Multi Learn File")))
		{
			ErrorPrint(eResult, "Failed to save\n");
			break;
		}
		...
		// 알고리즘 수행 // Execute the algorithm
		eResult = FLPatternMatchMultiSparseSave.Execute();
