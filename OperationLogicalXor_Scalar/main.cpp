...
int main()
{
	...
	do
	{
		...
		if((eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationLogicalOr/Cat.flif")).IsFail())
		{
		...
		// Logical Xor 값 설정 // Set Logical Xor value
		logicalXor.SetScalarValue(CMultiVar<double>(0, 0, 0));

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if((eResult = logicalXor.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation logical xor.");
			break;
		}
		...
		if((eResult = arrLayer[EType_Source].DrawTextCanvas(&CFLPointD(0, 0), L"Source Image", YELLOW, BLACK, 25)).IsFail())
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if((eResult = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPointD(0, 0), L"Destination1 Image(LogicalXor 0)", YELLOW, BLACK, 25)).IsFail())
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}

		if((eResult = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPointD(0, 0), L"Destination2 Image(LogicalXor 255)", YELLOW, BLACK, 25)).IsFail())
		{
			ErrorPrint(eResult, "Failed to draw text\n");
			break;
		}
