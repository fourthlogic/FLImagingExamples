...
int main()
{
	...
	do
	{
		...
		for(int32_t i = 0; i < 3; ++i)
		{
			...
			CFLPoint<double> pFlpGridPoint1;
			CFLPoint<double> pFlpGridPoint2;
			...
			int32_t i32LineTransition = 0;
			int32_t i32VertexNumber = 0;
			i32LineTransition = 0;
			...
			for(int64_t i64Row = 0; i64Row < i64GridRow; ++i64Row)
			{
				...
				{
					int64_t i64GridIdx = i64Row * i64GridCol + i64Col;
					uint32_t crTextColor = colorPool[(i32LineTransition++) % 3];

					if(!((i32VertexNumber + 1) % i64GridCol))
						crTextColor = YELLOW;
					else
					{
					...
					if(!i64Row)
					{
						...
						{
							pFlpGridPoint1 = ((CFLPoint<double>*)((*(CFLFigureArray*)sArrGridDisplay[i].sGridData.flfaGridData[i64Row + 1])[i64Col]));
							pFlpGridPoint2 = ((CFLPoint<double>*)((*(CFLFigureArray*)sArrGridDisplay[i].sGridData.flfaGridData[i64Row + 1])[i64Col + 1]));
						}
						else
						{
							pFlpGridPoint1 = ((CFLPoint<double>*)((*(CFLFigureArray*)sArrGridDisplay[i].sGridData.flfaGridData[i64Row + 1])[i64Col]));
							pFlpGridPoint2 = ((CFLPoint<double>*)((*(CFLFigureArray*)sArrGridDisplay[i].sGridData.flfaGridData[i64Row + 2])[0]));
						}
					...
					if(IsFail(res = layerLearn.DrawTextImage((CFLPoint<double>*)((*(CFLFigureArray*)sArrGridDisplay[i].sGridData.flfaGridData[i64Row])[i64Col]), CFLString<wchar_t>().Format(L"%d", i32VertexNumber++), crTextColor, BLACK, (float)(f64Pitch / 3), true, f64Angle)))
					{
						ErrorPrint(res, L"Failed to draw figure\n");
						break;
					}
					...
					if(i64Col)
						f64Angle = CFLPoint<double>((CFLPoint<double>*)((*(CFLFigureArray*)sArrGridDisplay[i].sGridData.flfaGridData[i64Row])[i64Col - 1])).GetAngle((CFLPoint<double>*)((*(CFLFigureArray*)sArrGridDisplay[i].sGridData.flfaGridData[i64Row])[i64Col]));
				}
