		...
		for(int32_t i = 0; i < 3; ++i)
		{
			...
			for(int64_t i64Row = 0; i64Row < i64GridRow; ++i64Row)
			{
				...
				for(int64_t i64Col = 0; i64Col < i64GridCol; ++i64Col)
				{
					...
					if(i64Col)
						f64Angle = CFLPoint<double>(sArrGridDisplay[i].sGridData.arrGridData[i64Row][i64Col - 1]).GetAngle(CFLPoint<double>(sArrGridDisplay[i].sGridData.arrGridData[i64Row][i64Col]));
				}
