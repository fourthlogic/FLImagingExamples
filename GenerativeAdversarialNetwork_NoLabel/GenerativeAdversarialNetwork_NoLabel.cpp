...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliLearnImage.Load(L"../../ExampleImages/GenerativeAdversarialNetwork/mnist1000.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(res = fliValidateImage.Load(L"../../ExampleImages/GenerativeAdversarialNetwork/mnist100.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		if(IsFail(res = viewImageResult.Create(1124, 0, 1636, 512)))
		{
			ErrorPrint(res, "Failed to create the image view.\n");
			break;
		}
		...
		if(IsFail(res = layerResult.DrawTextCanvas(&CFLPoint<double>(0, 0), L"INFERENCE", YELLOW, BLACK, 30)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
		...
		gan.SetModel(CGenerativeAdversarialNetworkDL::EModel_FLGenNet);
		// 학습할 Generative Adversarial Network 모델 설정 // Set up Generative Adversarial Network model to learn
		gan.SetModelVersion(CGenerativeAdversarialNetworkDL::EModelVersion_FLGenNet_V1_64);
		// 학습 epoch 값을 설정 // Set the learn epoch value 
		gan.SetLearningEpoch(300);
		...
		gan.SetInferenceResultCount(100);
		...
		while(viewImageLearn.IsAvailable() && viewImageResult.IsAvailable() && viewImageValidate.IsAvailable() && viewGraph.IsAvailable())
			CThreadUtilities::Sleep(1);
	}
	while(false);
