...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/SaliencyMap/LongTailedTit.flif")))
		{
		...
		if((eResult = SaliencyMap.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute SaliencyMap.");
			break;
		}
