...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(eResult = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationSine/Flower.flif")))
		{
		...
		if((eResult = sine.Execute()).IsFail())
		{
			ErrorPrint(eResult, "Failed to execute operation sine.");
			break;
		}
